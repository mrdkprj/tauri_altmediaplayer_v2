<script lang="ts">
    ...
    const shuffleList = () => {
        ...
        const target = new Array($appState.files.length).fill(undefined).map((_v, i) => i).filter(i => i !== $appState.currentIndex);
...
<div class="viewport">
    ...
    <div class="playlist-viewport" class:group-by={$appState.sortType.groupBy} bind:this={fileListContainer} role="button" tabindex="-1" on:dragover={e => e.preventDefault()}>
        {#if $appState.rename.renaming}
            ...
            />
        {/if}
        <List onPlaylistItemClicked={onPlaylistItemClicked} onMouseDown={onPlaylistItemMousedown} scrollToElement={scrollToElement} getChildIndex={getChildIndex}/>
    </div>
