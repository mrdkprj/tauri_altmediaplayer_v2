...
<div class="footer" on:mouseenter={onMouseEnter} role="button" tabindex="-1">
    <div class="footer-menu">
        <div class="footer-top">
            ...
            <div class="btn-area">
                <div class="btn-large" on:click={onClickPlay} on:keydown={handleKeyEvent} on:mouseenter={() => updatePreventAutohide(true)} on:mouseleave={() => updatePreventAutohide(false)} role="button" tabindex="-1">
                    {#if $appState.playing}
                ...
                <div class="btn" on:click={() => onClickPrevious(Buttons.left)} on:contextmenu={() => onClickPrevious(Buttons.right)} on:keydown={handleKeyEvent} on:mouseenter={() => updatePreventAutohide(true)} on:mouseleave={() => updatePreventAutohide(false)} role="button" tabindex="-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M.5 3.5A.5.5 0 0 0 0 4v8a.5.5 0 0 0 1 0V8.753l6.267 3.636c.54.313 1.233-.066 1.233-.697v-2.94l6.267 3.636c.54.314 1.233-.065 1.233-.696V4.308c0-.63-.693-1.01-1.233-.696L8.5 7.248v-2.94c0-.63-.692-1.01-1.233-.696L1 7.248V4a.5.5 0 0 0-.5-.5z"/>
                    </svg>
                </div>
                <div class="btn" on:click={() => onClickNext(Buttons.left)} on:contextmenu={() => onClickNext(Buttons.right)} on:keydown={handleKeyEvent} on:mouseenter={() => updatePreventAutohide(true)} on:mouseleave={() => updatePreventAutohide(false)} role="button" tabindex="-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
