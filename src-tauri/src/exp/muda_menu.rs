...
use tauri::menu::{CheckMenuItem, CheckMenuItemBuilder, IsMenuItem, Menu, MenuBuilder, MenuId, MenuItemBuilder, Submenu, SubmenuBuilder};
...
fn to_items<R: tauri::Runtime>(items: &[Box<dyn IsMenuItem<R>>]) -> Vec<&dyn IsMenuItem<R>> {
...
fn toggle_checked<R: tauri::Runtime>(menu_king: MenuKind<Wry>, checked_menu: &CheckMenuItem<R>) {
    ...
    items.iter().filter(|item| !MenuIdExt::get(item.id()).id.is_empty()).filter(|item| item.id() != EXCEPT_ID).for_each(|item| {
...
pub fn create_player_menu(handle: &tauri::AppHandle, window: &tauri::WebviewWindow, settings: &Settings) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(handle).item(&playback_speed).item(&seekSpeed).item(&fitToWindow).separator().item(&playlist).item(&fullscreen).item(&pip).separator().item(&capture).separator().item(&theme).build()?;
...
pub fn create_playlist_menu(handle: &tauri::AppHandle, window: &tauri::WebviewWindow) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(handle).item(&remove).item(&trash).separator().item(&copyName).item(&copyFullpath).item(&reveal).separator().item(&rename).item(&metadata).item(&convert).separator().item(&addTag).item(&manageTag).separator().item(&loadList).item(&saveList).separator().item(&clearList).build()?;
...
pub fn create_sort_menu(handle: &tauri::AppHandle, window: &tauri::WebviewWindow, settings: &Settings) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(handle).item(&groupBy).separator().items(&to_items(&menu_items)).build()?;
